#notes for processing the Trawl 2018 12S data
#date: Dec 8th, 2020
#working directory: ~/projects/12S_runs/trawl_2018/

#before running R, copy data from NAS to project folder
#data processing for ICO 12S plate 1
#no data prep, as it's already demultiplexed coming off the MiSeq. can move straight to R processing with dada2
rsync -auP /mnt/MiSeq/12S_TRAWL_Run20201204/Run20201204/Alignment_1/20201207_065819/Fastq/* ~/projects/12S_runs/trawl_2018/raw_data/
...
####Environment Setup####
theme_set(theme_bw())
setwd("~/projects/12S_runs/trawl_2018/")

####File Path Setup####
#this is so dada2 can quickly iterate through all the R1 and R2 files in your read set
path <- "~/projects/12S_runs/trawl_2018/raw_data/test" # CHANGE ME to the directory containing the fastq files
list.files(path)
...
####fastq Quality Plots####
pdf("quality_plots.dada2.R1s.test_set.pdf", width = 16, height = 9) # define plot width and height. completely up to user.
  plotQualityProfile(fnFs[1:14]) #this plots the quality profiles for each sample, if you have a lot of samples, it's best to look at just a few of them, the plots take a minute or two to generate even only showing 10-20 samples.
dev.off()
pdf("quality_plots.dada2.R2s.test_set.pdf", width = 16, height = 9) # define plot width and height. completely up to user.
  plotQualityProfile(fnRs[1:14])
dev.off()
#should trim off first 25 reads and last 100bp, based on quality plots. there are also many failed samples

####running primer removal test on subset of data####
FWD <- "GGWACWGGWTGAACWGTWTAYCCYCC"  ## CHANGE ME to your forward primer sequence
REV <- "TAAACTTCAGGGTGACCAAARAAYCA"  ## CHANGE ME to your reverse primer sequence
...
filterAndTrim(fnFs, fnFs.filtN, fnRs, fnRs.filtN, trimLeft=c(25,25), trimRight=c(100,100), truncQ=c(2,2), maxN = 0, multithread = TRUE, compress = TRUE, matchIDs=TRUE)
...
index <- 2 #this is the index of the file we want to check for primers, within the lists "fn*s.filtN", it can be any number from 1 to N, where N is the number of samples you are processing
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, truncLen=c(0,0), truncQ=c(2,2), minLen = c(110,110),
                     maxN=c(0,0), maxEE=c(8,10), rm.phix=TRUE, matchIDs=TRUE,
...
#[1] 0.962514 data looks to be good quality w/r/t chimeras
...
write.table(data.frame("row_names"=rownames(track),track),"read_retention.12S_merged.txt", row.names=FALSE, quote=F, sep="\t")
write.table(data.frame("row_names"=rownames(seqtab.nosingletons.nochim),seqtab.nosingletons.nochim),"sequence_table.12S.merged.txt", row.names=FALSE, quote=F, sep="\t")
...
write.fasta(sequences=as.list(ASV.seq), names=ASV.num, "12S_ASV_sequences.fasta") #save sequences with new names in fasta format
...
write.table(data.frame("row_names"=rownames(seqtab.nosingletons.nochim),seqtab.nosingletons.nochim),"sequence_table.12S.merged.w_ASV_names.txt", row.names=FALSE, quote=F, sep="\t")
...
mkdir blast_96_sim
blastn -task megablast -num_threads 38 -evalue 1e-5 -max_target_seqs 10 -perc_identity 96 -qcov_hsp_perc 50 -db ~/projects/taxonomyDBs/NCBI_NT/2021-11-05/nt -outfmt '6 qseqid stitle sacc staxid pident qcovs evalue bitscore' -query 12S_ASV_sequences.fasta  -out blast_96_sim/12S_ASV_sequences.length_var.blast.out
